...
export const AdminPanel: React.FC<AdminPanelProps> = ({ onClose, onEdit, onAdd }) => {
  ...
  const [regularPeriod, setRegularPeriod] = useState<'daily' | 'weekly' | 'monthly'>('daily');
  ...
  const inSelectedBucket = (q: Quest) => {
    ...
    if (regularPeriod === 'daily') return isBetween(d, dayRange.start, dayRange.end);
    if (regularPeriod === 'weekly') return isBetween(d, weekRange.start, weekRange.end);
          ...
          <div className="sticky top-0 z-10 bg-gradient-to-r from-emerald-600 to-blue-600 text-white">
            <div className="flex items-center justify-between px-3 py-2">
              <div className="flex gap-2">
                <motion.button whileTap={{ scale: 0.96 }} onClick={() => setTab('regular')}
                  className={`px-3 py-1.5 rounded-full text-sm transition ${tab === 'regular' ? 'bg-white/90 text-slate-900 shadow' : 'bg-white/10'}`}>
                  Обычные
                </motion.button>
                <motion.button whileTap={{ scale: 0.96 }} onClick={() => setTab('events')}
                  className={`px-3 py-1.5 rounded-full text-sm transition ${tab === 'events' ? 'bg-white/90 text-slate-900 shadow' : 'bg-white/10'}`}>
                  Ивенты
                </motion.button>
              </div>
              <div className="flex items-center gap-2">
                <motion.button whileTap={{ scale: 0.96 }} onClick={onAdd}
                  className="px-3 py-1.5 rounded-full text-sm bg-white/95 text-slate-900 shadow">+ Добавить</motion.button>
                <motion.button whileTap={{ scale: 0.9, rotate: 90 }} onClick={onClose} aria-label="Закрыть"
                  className="w-9 h-9 inline-flex items-center justify-center rounded-full bg-white/15 hover:bg-white/25">×</motion.button>
              </div>
            </div>
              ...
              <div className="px-3 pb-3">
                <div className="flex gap-2 mb-2">
                  {(['daily', 'weekly', 'monthly'] as const).map((p) => (
                    <motion.button key={p} whileTap={{ scale: 0.96 }} onClick={() => setRegularPeriod(p)}
                      className={`px-3 py-1.5 rounded-full text-sm transition ${regularPeriod === p ? 'bg-white text-slate-900 shadow' : 'bg-white/15 text-white'}`}>
                      {p === 'daily' ? 'Ежедневные' : p === 'weekly' ? 'Еженедельные' : 'Ежемесячные'}
                    </motion.button>
                  ))}
                </div>
                <div className="flex items-center gap-2">
                  ...
                  {regularPeriod === 'weekly' && (
                    <input type="week" value={weekISO} onChange={(e) => setWeekISO(e.target.value)}
                      className="px-3 py-2 rounded-lg w-full bg-white/95 text-slate-800 shadow-inner" />
                  )}
      ...
      <AnimatePresence initial={false}>
        {active.length > 0 && (
          <motion.div key="active" initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -6 }} transition={{ duration: 0.2 }}>
            <h4 className="text-xs font-semibold text-emerald-700 mb-2">Активные</h4>
            <motion.div layout className="space-y-2" initial="hidden" animate="show" variants={{ hidden: {}, show: { transition: { staggerChildren: 0.04 } } }}>
              {active.map((q) => (
                <motion.div key={q.id} layout initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -8 }} transition={{ type: 'spring', stiffness: 260, damping: 24 }}>
                  <QuestCard quest={q} isAdmin onEdit={() => onEdit(q)} onDelete={() => onDelete(q.id)} />
                </motion.div>
              ))}
            </motion.div>
          </motion.div>
        ...
        {scheduled.length > 0 && (
          <motion.div key="scheduled" initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -6 }} transition={{ duration: 0.2 }}>
            <h4 className="text-xs font-semibold text-slate-700 mb-2">Запланированные</h4>
            <motion.div layout className="space-y-2" initial="hidden" animate="show" variants={{ hidden: {}, show: { transition: { staggerChildren: 0.04 } } }}>
              {scheduled.map((q) => (
                <motion.div key={q.id} layout initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -8 }} transition={{ type: 'spring', stiffness: 260, damping: 24 }}>
                  <QuestCard quest={q} isAdmin onEdit={() => onEdit(q)} onDelete={() => onDelete(q.id)} />
        ...
        {active.length === 0 && scheduled.length === 0 && (
          <motion.p key="empty" initial={{ opacity: 0 }} animate={{ opacity: 0.8 }} className="text-slate-500 text-center py-8">
            {emptyText}
          </motion.p>
        )}
      </AnimatePresence>
